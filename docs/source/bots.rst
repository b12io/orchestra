...
* ``orchestra.communication.staffing.address_staffing_requests`` looks for tasks that can be staffed and reaches out to the next set of ``settings.ORCHESTRA_STAFFBOT_WORKER_BATCH_SIZE`` workers every ``settings.ORCHESTRA_STAFFBOT_BATCH_FREQUENCY``.
